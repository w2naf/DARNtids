...
#            img2 = 'check'
        ...
        anc1_tag  = '<a href="javascript:plotRTI(\''+x['radar']+'\',\''+xstr+'\',\'power\');">'
...
#    items  = [x for x in db['listTracker'].find().sort('name',1)]
    ...
    for x in db['listTracker'].find().sort('name',1):
        if x['name'] not in db.collection_names():
    ...
    html = []
    html.append('<select name="list_dropdown" id="list_dropdown">')
...
#    html.append('  <option value="clean">--Clean List--</option>')
    ...
    for x in items:
        ...
        html.append(tag)
    html.append('</select>')
    html.append('<button id="listLoadButton">Load</button>')
#    html.append('<button id="listSaveButton">Save</button>')
    html.append('<button id="listDeleteButton">Delete</button>')
    html = '\n'.join(html)
    return html
...
def set_nav_mode(nav_mode):
    db['settings'].remove({'nav_mode':{'$exists':True}})
